...
class Music
{
     ...
     * Ignora arquivos protegidos como promo.mp3.
     */
    public function getOrphanFiles(): array
    {
        try {
            // 1. Coleta caminhos registrados (Musicas e Promos)
            $validos = [];
            
            // Tabela musicas
            $stmt = $this->conn->query("SELECT caminho_arquivo, caminho_imagem FROM {$this->table}");
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if (!empty($row['caminho_arquivo'])) {
                    $rp = realpath(__DIR__ . '/../' . $row['caminho_arquivo']);
                    if ($rp) $validos[] = $rp;
                }
                if (!empty($row['caminho_imagem'])) {
                    $rp = realpath(__DIR__ . '/../' . $row['caminho_imagem']);
                    if ($rp) $validos[] = $rp;
                }
            }

            // Tabela promos (se existir)
            try {
                ...
                // Tabela promo pode não existir ainda
            }
            ...
            // Configuração das pastas conforme a estrutura do projeto
            $pastas = [
                'music/'        => __DIR__ . '/../music/',
                'music/covers/' => __DIR__ . '/../music/covers/',
                'promo/'        => __DIR__ . '/../promo/'
            ];
            ...
            foreach ($pastas as $prefixo => $caminhoAbsoluto) {
                ...
                $arquivos = glob($caminhoAbsoluto . '{*.mp3,*.jpg,*.jpeg,*.png,*.gif,*.webp}', GLOB_BRACE) ?: [];
                
                foreach ($arquivos as $arquivo) {
                    $nomeBase = basename($arquivo);
                    $rp = realpath($arquivo);

                    // Verifica se o arquivo não está no banco e não é protegido
                    if ($rp && !in_array($rp, $validos) && !in_array($nomeBase, $protegidos)) {
