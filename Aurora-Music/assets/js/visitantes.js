 ...
 * Gerenciamento de Estatísticas com suporte a Tabela e Cards
 */
...
async function carregarTabelaVisitantes(dataInicio, dataFim, pagina) {
    ...
    const cardContainer = document.getElementById('visitanteCards');
    ...
    try {
        ...
        if (data.success) {
            // Renderiza na tabela (Desktop)
            if (tbody) renderizarTabela(data.dados);
            
            // Renderiza em cards (Mobile)
            if (cardContainer) renderVisitorCards(data.dados);
...
function renderVisitorCards(visitantes) {
    const container = document.getElementById('visitanteCards');
    if (!container) return;

    if (!visitantes || visitantes.length === 0) {
        container.innerHTML = '<p style="color:#94a3b8;text-align:center;padding:2rem 0;grid-column:1/-1;">Nenhum visitante registrado.</p>';
        return;
    }
    ...
    container.innerHTML = visitantes.map(v => `
        <div class="visitor-card">
            ...
            <div class="visitor-card-delete">
                <button class="btn-delete" onclick="excluirRegistro(${v.id})" title="Excluir visitante">
                    <i class='bx bx-trash'></i>
                </button>
            </div>
        </div>
