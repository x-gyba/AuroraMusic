...
const player = {
    ...
    // --- Configurações de Promoção Dinâmica ---
    songsPlayed: 0,                
    isPromoPlaying: false,         
    promoPlaylist: [], // Será preenchida via fetch no Controller

    async init() {
        console.log('[AURORA] Iniciando Player...');
        history.replaceState(null, '', window.location.pathname + window.location.search);

        // 1. Carregar lista de promos da pasta /promo/ via Controller
        await this.loadPromos();

        // 2. Setup padrão do player
        this.cacheElements();
        this.setupPlaylist();
    ...
    // Varre a pasta de promos chamando o controller PHP
    async loadPromos() {
        try {
            const response = await fetch('controllers/get_promos.php');
            this.promoPlaylist = await response.json();
            console.log('[AURORA] Promos sincronizadas:', this.promoPlaylist);
        } catch (e) {
    ...
    /* Lógica para tocar a Promoção Aleatória */
    playPromo() {
        // Se não houver músicas no player (conforme foto), não tenta tocar promo
        if (this.playlist.length === 0 || this.promoPlaylist.length === 0) {
            console.log('[AURORA] Condição para promo não atingida.');
            return;
        }

        console.log('[AURORA] Escolhendo anúncio aleatório...');
        this.isPromoPlaying = true;
        
        // Sorteia um arquivo da pasta promo/ que o Controller encontrou
    ...
    nextTrack() {
        // Finaliza promo e volta para a playlist
        ...
        // Se a playlist principal estiver vazia (foto), não faz nada
        if (this.playlist.length === 0) return;

        this.songsPlayed++;

        // A cada 3 músicas toca um anúncio aleatório
