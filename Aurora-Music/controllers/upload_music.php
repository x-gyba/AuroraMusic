...
/* ══════════════════════════════════════════════════════════════
   PARSER ID3v2 ROBUSTO
   Suporta v2.2, v2.3, v2.4 — lida com encoding UTF-16
   ══════════════════════════════════════════════════════════════ */
function extractEmbeddedCover(string $mp3path): ?array {
    ...
    // Pula extended header
    if ($hasExtHeader && $majorVersion >= 3) {
        if ($majorVersion === 4) {
            $es = ((ord($tagData[0]) & 0x7F) << 21) | ((ord($tagData[1]) & 0x7F) << 14)
                | ((ord($tagData[2]) & 0x7F) <<  7) |  (ord($tagData[3]) & 0x7F);
        } else {
            $es = (ord($tagData[0]) << 24) | (ord($tagData[1]) << 16)
                | (ord($tagData[2]) << 8)  |  ord($tagData[3]);
        }
        $pos += $es;
    }
    ...
    while ($pos < $len - 10) {
        if ($majorVersion === 2) {
            ...
            $frameSize = (ord($tagData[$pos+3]) << 16)
                       | (ord($tagData[$pos+4]) << 8)
                       |  ord($tagData[$pos+5]);
            $pos += 6;
            $apicTag = 'PIC';
            ...
            if ($majorVersion === 4) {
                $frameSize = ((ord($tagData[$pos+4]) & 0x7F) << 21)
                           | ((ord($tagData[$pos+5]) & 0x7F) << 14)
                           | ((ord($tagData[$pos+6]) & 0x7F) <<  7)
                           |  (ord($tagData[$pos+7]) & 0x7F);
            } else {
                $frameSize = (ord($tagData[$pos+4]) << 24)
                           | (ord($tagData[$pos+5]) << 16)
                           | (ord($tagData[$pos+6]) << 8)
                           |  ord($tagData[$pos+7]);
            }
            $pos += 10;
        ...
        if ($frameId === 'PIC') {
            ...
            $ext  = ($fmt === 'jpg' || $fmt === 'jpeg') ? 'jpg'
                  : ($fmt === 'png' ? 'png' : ($fmt === 'gif' ? 'gif'
                  : ($fmt === 'webp' ? 'webp' : null)));
            if (!$ext) continue;
            $fpos++;
            ...
            $mime = '';
            while ($fpos < strlen($frameData) && $frameData[$fpos] !== "\x00") {
                $mime .= $frameData[$fpos++];
            }
            $fpos++;
...
try {
    ...
    if (!$musicModel->checkStorageLimit($userId, 500 * 1024 * 1024)) {
    ...
    foreach ([0755, 0775, 0777] as $perm) {
        if (is_writable($uploadDir)) break;
        @chmod($uploadDir, $perm);
        clearstatcache(true, $uploadDir);
    }

    if (!is_writable($uploadDir)) {
        throw new Exception('Diretório de upload não gravável. Ajuste permissões da pasta "music/".');
    }
    ...
    if (!@move_uploaded_file($fileTmpPath, $destinationAbsolutePath)) {
        throw new Exception('Erro ao mover arquivo. Verifique permissões e espaço em disco.');
    }

    // ── Nome de exibição ───────────────────────────────────────
    $displayNameFromPost = isset($_POST['displayName']) ? trim($_POST['displayName']) : '';
    $nomeExibicao = !empty($displayNameFromPost)
        ? $displayNameFromPost
        : pathinfo($fileName, PATHINFO_FILENAME);

    // ── Cover: upload explícito > embutido no MP3 ──────────────
    $caminhoImagem = null;
    $tipoImagem    = null;
    $coverSource   = null;
    ...
    if (!$musicModel->save($musicData)) {
        if (file_exists($destinationAbsolutePath)) unlink($destinationAbsolutePath);
        throw new Exception('Erro ao salvar no banco de dados.');
    }

    $msg = 'Música enviada com sucesso!';
    if ($coverSource === 'embedded') $msg .= ' (capa extraída do MP3)';
    if ($coverSource === 'uploaded')  $msg .= ' (capa enviada)';
