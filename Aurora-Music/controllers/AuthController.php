 ...
 */
function registrarLoginHistorico($conn, $userId, $nome, $email, $ip) {
    try {
        $stmt = $conn->prepare("
            INSERT INTO login_historico (usuario_id, nome, email, ip_address, data_login)
            VALUES (:uid, :nome, :email, :ip, NOW())
        ");
        $stmt->execute([
            ':uid'   => $userId,
            ':nome'  => $nome,
        ...
        error_log("Erro ao registrar histórico de login: " . $e->getMessage());
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_GET['auth_action'] ?? '') === 'login') {
    ...
    try {
        ...
        registrarLoginHistorico($conn, $user->id, $user->nome, $user->email, $ip);
        ...
        echo json_encode([
            ...
            'redirect' => '/Aurora-Music/dashboard',
            'user'     => [
                'id'    => $user->id,
                'nome'  => $user->nome,
                'email' => $user->email
